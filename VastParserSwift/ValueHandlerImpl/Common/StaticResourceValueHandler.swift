//
//  StaticResourceValueHandler.swift
//  VastParserSwift
//
...
class StaticResourceValueHandler: ValueHandler {
    ...
    func execute(state: ParseState, value: String) {
        ...
        if self.rank == 8 {
            ...
            if state.checkAnyParent(rank: self.rank-1, targetElementName: "Icon") {
                // InLine Only
                if state.checkAnyParent(rank: self.rank-5, targetElementName: "InLine") {
                    
                    state.vast.vastAd?.vastInline?.creatives.creative.last?.linear?.icons?.icon.last?.staticResource.last?.uri = value
                    
                } else {
                    state.errors.append(ParseError(message: "StaticResource should be under Icon of InLine"))
                }
                
            } else {
                state.errors.append(ParseError(message: "StaticResource should be under Icon"))
            }
    ...
    private func inlineHandler(state: ParseState, value : String) {
        if state.checkAnyParent(rank: self.rank-1, targetElementName: "NonLinear") {
            ...
            state.vast.vastAd?.vastInline?.creatives.creative.last?.nonlinarads?.nonlinear.last?.staticResource.last?.uri = value
            
        } else if state.checkAnyParent(rank: self.rank-1, targetElementName: "Companion") {
            
            state.vast.vastAd?.vastInline?.creatives.creative.last?.companionAds?.companion.last?.staticResource.last?.uri = value
    ...
    private func wrapperHandler(state: ParseState, value : String) {
        if state.checkAnyParent(rank: self.rank-1, targetElementName: "NonLinear") {
            ...
            state.vast.vastAd?.wrapper?.creatives?.creative.last?.nonlinarads?.nonlinear.last?.staticResource.last?.uri = value
            
        } else if state.checkAnyParent(rank: self.rank-1, targetElementName: "Companion") {
        
            state.vast.vastAd?.wrapper?.creatives?.creative.last?.companionAds?.companion.last?.staticResource.last?.uri = value
